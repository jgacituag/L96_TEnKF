...
def assimilation_letkf_run(conf):
    ...
    Nx = ModelConf["nx"]       # Get the size of the state vector
    NxSS = ModelConf["nxss"]   # Get the size of the small-scale state
    NEns = DAConf["NEns"]      # Get the number of ensembles
    ...
    Departure = np.zeros([Nx, DALength])
    ...
    PreCycles   = int(DAConf.get("PreSpinupCycles", 0))
    PreInflFact = float(DAConf.get("PreSpinupInflation", 1.0))

    if PreCycles > 0:
        print( f"Pre-spinup: {PreCycles} free cycles with multiplicative inflation={PreInflFact}")

        stateens = np.copy(XA[:, :, 0])

        for ipre in range(PreCycles):
            ntout = int(DAConf["Freq"] / DAConf["TSFreq"]) + 1
            XFtmp, XSStmp, DFtmp, RFtmp, SSFtmp, CRFtmp, CFtmp = model.tinteg_rk4(
                nens=NEns,
                nt=DAConf["Freq"],
                ntout=ntout,
                x0=stateens,
    ...
    for it in range(1, DALength):
        ...
        if np.any(np.isnan(XA[:, :, it - 1])) or np.any(np.isinf(XA[:, :, it - 1])):
            ...
            print(f"Error: The analysis contains NaN or Inf, Iteration number :{it} will dump some variables to a temporal file")
            NormalEnd = False
            break
        ...
        if np.any(np.isnan(XFtmp[:, :, -1])) or np.any(np.isinf(XFtmp[:, :, -1])):
            # Stop the cycle before the fortran code hangs because of NaNs
            print(f"Error: The forecast contains NaN or Inf, Iteration number :{it}")
            NormalEnd = False
            break
        ...
        window_mask = np.logical_and(
            ObsLoc[:, 1] > da_window_start, ObsLoc[:, 1] <= da_window_end)
        ...
        for itemp in range(DAConf["NTemp"]):
            ...
            if NObsW > 0:
                ...
                [YF, YFqc] = hoperator.model_to_obs(
                    nx=Nx,
                    no=NObsW,
                    nt=1,
                    nens=NEns,
                    ...
                    xloc=ModelConf["XLoc"],
                    tloc=TLoc,
                    gross_check_factor=DAConf["GrossCheckFactor"],
            ...
            if BridgeParam < 1.0:
                ...
                if NObsWStep > 0:
                    d_out = np.empty(NObsWStep, dtype=np.float64)
                    xa = das.da_letkf(
                        nx=Nx,
                        nt=1,
                        no=NObsWStep,
